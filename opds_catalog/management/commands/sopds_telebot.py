...
class Command(BaseCommand):
    ...
    def getBooks(self, bot, update):
        ...
        bot.send_message(chat_id=update.message.chat_id, text=response['message'], parse_mode='HTML',
    ...
    def getBooksPage(self, bot, update):
        ...
        bot.edit_message_text(chat_id=callback_query.message.chat_id, message_id=callback_query.message.message_id,
                              text=response['message'], parse_mode='HTML', reply_markup=response['buttons'])
        return
    ...
    @cmdtrans
    @CheckAuthDecorator
    def uploadBook(self, bot: Bot, update: Update):
        name = update.message.document.file_name
        full_path = config.SOPDS_ROOT_LIB
        file = os.path.join(full_path, name)

        if os.path.exists(file):
            self.logger.info(f"File {file} already exists.")
            response = f"{_('File with name already exists')}: {name}"
            bot.sendMessage(chat_id=update.message.chat_id, text=response)
            return

        self.logger.info(f"File {file} download start.")
        bot.get_file(update.message.document).download()

        with open(file, 'wb') as f:
            bot.get_file(update.message.document).download(out=f)

        self.logger.info("File download complete.")

        response = f"{_('Received file with name')}: {name}"
        bot.sendMessage(chat_id=update.message.chat_id, text=response)

        self.logger.info("Start file scan")
        file_size = os.path.getsize(file)
        scanner = opdsScanner(self.logger)
        with transaction.atomic():
            scanner.processfile(name, full_path, file, None, 0, file_size)
        Counter.objects.update_known_counters()

        response = f"{_('File with name')}: {name} {_('is uploaded')}"
        bot.sendMessage(chat_id=update.message.chat_id, text=response)
    ...
    def start(self):
        ...
        try:
            ...
            upload_handler = MessageHandler(Filters.document, self.uploadBook)
